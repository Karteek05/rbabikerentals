...
function getGoogleMapsKey() {
    ...
    if (!key) {
        throw new errors_1.ApiException(500, "google_maps_env_missing", "GOOGLE_MAPS_API_KEY is missing.");
    }
    return key;
}
...
async function reverseGeocode(input) {
    ...
    if (!response.ok) {
        throw new errors_1.ApiException(502, "google_maps_reverse_geocode_failed", "Google Maps reverse geocode request failed.");
    }
    const payload = (await response.json());
    if (payload.status !== "OK" && payload.status !== "ZERO_RESULTS") {
        throw new errors_1.ApiException(502, "google_maps_reverse_geocode_failed", payload.error_message ?? `Google Maps returned ${payload.status}.`);
    }
    return {
...
async function distanceMatrix(input) {
    ...
    const response = await fetch(url);
    if (!response.ok) {
        throw new errors_1.ApiException(502, "google_maps_distance_failed", "Google Maps distance request failed.");
    }
    const payload = (await response.json());
    if (payload.status !== "OK") {
        throw new errors_1.ApiException(502, "google_maps_distance_failed", payload.error_message ?? `Google Maps returned ${payload.status}.`);
    }
    const element = payload.rows?.[0]?.elements?.[0];
    if (!element || element.status !== "OK") {
        throw new errors_1.ApiException(422, "route_unavailable", "No route is available for the requested coordinates.");
    }
    return {
        mode,
