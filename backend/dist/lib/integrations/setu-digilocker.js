...
exports.createDigilockerRequest = createDigilockerRequest;
exports.fetchDigilockerRequestStatus = fetchDigilockerRequestStatus;
...
function getSetuConfig() {
    ...
    if (!baseUrl || !clientId || !clientSecret || !productInstanceId || !redirectUrl) {
        throw new errors_1.ApiException(500, "setu_env_missing", "Setu DigiLocker env vars are not fully configured.");
    }
    return { baseUrl, clientId, clientSecret, productInstanceId, redirectUrl };
}
async function createDigilockerRequest() {
    ...
    if (!response.ok) {
        throw new errors_1.ApiException(502, "setu_request_failed", "Setu DigiLocker request creation failed.");
    }
    return response.json();
}
async function fetchDigilockerRequestStatus(requestId) {
    const cfg = getSetuConfig();
    ...
    if (!response.ok) {
        throw new errors_1.ApiException(502, "setu_status_failed", "Setu DigiLocker status fetch failed.");
    }
    return response.json();
}
