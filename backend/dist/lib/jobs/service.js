...
exports.runDocumentExpiryReminderJob = runDocumentExpiryReminderJob;
...
async function runDocumentExpiryReminderJob() {
    const reminderCutoff = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const docs = await (0, repository_1.listVehicleDocumentsExpiringBefore)(reminderCutoff);
    for (const doc of docs) {
        await (0, repository_1.insertNotificationJob)({
            id: (0, ids_1.newId)("notif"),
            channel: "whatsapp",
...
async function runIncidentEscalationJob() {
    const incidents = await (0, repository_1.listOpenDamageIncidents)();
