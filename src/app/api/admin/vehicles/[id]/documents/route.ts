...
import { readValidatedUpload } from "@/lib/uploads/validation";
import {
...
async function uploadFile(
	...
	const { buffer, fileName } = await readValidatedUpload(file, {
		kind: "document",
	...
	if (isSupabaseConfigured()) {
		const bucket =
			process.env.SUPABASE_VEHICLE_DOC_BUCKET ?? "vehicle-documents";
...
export async function POST(
	...
	try {
		...
		if (
			typeof docTypeEntry !== "string" ||
			!["rc", "insurance", "invoice"].includes(docTypeEntry)
		) {
			throw new ApiException(
				400,
				"invalid_doc_type",
