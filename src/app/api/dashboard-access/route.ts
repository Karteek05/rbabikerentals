...
function issueCookie(role: NonNullable<ReturnType<typeof dashboardRoleFromParam>>) {
...
async function setAccessCookie(role: NonNullable<ReturnType<typeof dashboardRoleFromParam>>) {
...
export async function POST(request: Request) {
  ...
  if (!role) {
    return NextResponse.json(
      { ok: false, error: { code: "invalid_request", message: "Role is required." } },
      { status: 400 }
    );
  }
  ...
  if (body.action === "verify_code") {
    ...
    if (!saved || saved.expiresAt <= Date.now()) {
      ...
      return NextResponse.json(
        { ok: false, error: { code: "code_expired", message: "Email code expired. Send a new code." } },
        { status: 401 }
      );
    }
    if (saved.attempts >= 5 || !body.code || saved.code !== body.code.trim()) {
      emailCodes.set(codeKey(role, request), { ...saved, attempts: saved.attempts + 1 });
      recordFailedAttempt(clientKey);
      return NextResponse.json(
        { ok: false, error: { code: "invalid_code", message: "Invalid email code." } },
        { status: 401 }
      );
    }
    emailCodes.delete(codeKey(role, request));
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: "Dashboard email is not configured. Add ADMIN_DASHBOARD_EMAIL or PARTNER_DASHBOARD_EMAIL."
          }
        },
        { status: 500 }
      );
