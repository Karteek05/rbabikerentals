...
export function resolveKycCallbackStatus(input: {
  ...
  const authenticated =
    normalized === "authenticated" || normalized === "verified" || normalized === "success";
  ...
  if (normalized === "unauthenticated" || normalized === "in_progress" || normalized === "initiated") {
    ...
    if (
      collection.some((doc) => {
        ...
        if (doc && typeof doc === "object") {
          ...
          const type = String(record.docType ?? record.doc_type ?? record.type ?? "").toUpperCase();
...
export function parseSetuDigilockerStatus(payload: Record<string, unknown>) {
  ...
  const userDetails =
    payload.digilockerUserDetails && typeof payload.digilockerUserDetails === "object"
