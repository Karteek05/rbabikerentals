...
import {
	...
	updatePaymentOrderRefundIfUnchanged,
...
function toUpiFallbackOrder(params: { bookingId: string; amount: number }) {
	return {
...
async function waitForFinalizedBooking(bookingId: string) {
	for (let attempt = 0; attempt < 5; attempt += 1) {
		...
		if (
			...
			currentBooking.status === "extended"
		) {
...
async function finalizeCapturedPayment(params: {
	updatedOrder: NonNullable<
		Awaited<ReturnType<typeof updatePaymentOrderByProviderId>>
	>;
	paidAmount?: number;
}) {
	...
	if (booking.status === "extension_requested") {
		...
		assertCanTransition(
			"extension_requested",
			"extended",
			"booking.extend.confirm",
		);
		const updatedBooking = await updateBookingIfStatus(
			booking.id,
			"extension_requested",
			{
				status: "extended",
		...
		const email = await resolveUserNotificationEmail(
...
async function syncOpenPaymentOrderForBooking(bookingId: string) {
	try {
		...
		const capturedPayment = await fetchCapturedPaymentForOrder(
			openOrder.provider_order_id,
		);
		...
		const updatedOrder = await updatePaymentOrderByProviderId(
			openOrder.provider_order_id,
			{
...
export async function confirmRazorpayCheckoutPayment(params: {
	...
	const ownerAllowed =
		params.actor.role === "customer" && booking.user_id === params.actor.userId;
	if (!ownerAllowed && params.actor.role !== "admin") {
		throw new ApiException(
			403,
			"forbidden",
			"Not allowed to confirm payment for this booking.",
		);
	}
	...
	const confirmedBooking = await finalizeCapturedPayment({
		updatedOrder,
		paidAmount: await resolveCapturedAmount(
			params.orderId,
			updatedOrder.amount,
		),
	});
	...
	if (
		...
		confirmedBooking.status !== "extended"
	) {
		throw new ApiException(
			409,
...
export async function createOrderForBooking(
	...
	const ownerAllowed =
		actor.role === "customer" && booking.user_id === actor.userId;
	if (!ownerAllowed && actor.role !== "admin") {
		throw new ApiException(
			403,
			"forbidden",
			"Not allowed to create a payment order for this booking.",
		);
	}
	if (booking.status !== "payment_pending") {
		throw new ApiException(
	...
	if (isRazorpayConfigured()) {
		...
		if (
			...
			isCheckoutOrderId(paidOrder.provider_order_id)
		) {
			try {
				await finalizeCapturedPayment({
	...
	try {
		...
		if (
			...
			error.code === "payment_order_exists"
		) {
			const openOrder = await getOpenPaymentOrderForBooking(bookingId);
			if (openOrder) {
				return toClientOrder({
...
export async function processRazorpayWebhook(params: {
	...
	if (payload.event === "payment_link.paid") {
		...
		const description =
			payload.payload?.payment_link?.entity?.description ?? "";
		...
		if (paymentLinkId) {
			...
			if (!updatedOrder) {
				const match = (referenceId + " " + description).match(
					/(booking_[a-zA-Z0-9-]+)/,
				);
				if (match && match[1]) {
...
export async function refundBookingAmount(params: {
	...
	const listedPaymentOrders = await listPaymentOrdersForBooking(
		params.bookingId,
	);
	const paymentOrders = listedPaymentOrders.filter(
	...
	if (paymentOrders.length === 0) {
		throw new ApiException(
			...
			"Booking does not have a paid payment order.",
		);
	}
...
export async function issueBookingRefund(params: {
	...
	const booking = await getBookingOrThrow(params.bookingId);
	let paymentOrder = await getLatestPaymentOrderForBooking(params.bookingId);
	if (params.paymentOrderId) {
		const listedPaymentOrders = await listPaymentOrdersForBooking(
			params.bookingId,
		);
		paymentOrder =
			listedPaymentOrders.find((order) => order.id === params.paymentOrderId) ??
			null;
	}
	if (
		!paymentOrder ||
	...
	const totalReserved = alreadyRefunded + params.amountInPaise;
	const reservation = await updatePaymentOrderRefundIfUnchanged(
		paymentOrder.id,
		alreadyRefunded,
		{
			refunded_amount: totalReserved,
			status: totalReserved >= paymentOrder.amount ? "refunded" : "paid",
			updated_at: new Date().toISOString(),
		},
	);
	if (!reservation) {
		throw new ApiException(
			409,
			"refund_conflict",
			"Refund balance changed; retry request.",
		);
	}

	let refund: Awaited<ReturnType<typeof createRazorpayRefund>>;
	try {
		refund = await createRazorpayRefund({
			...
			idempotencyKey: `refund_${paymentOrder.id}_${alreadyRefunded}_${params.amountInPaise}`,
		});
	} catch (error) {
		await updatePaymentOrderRefundIfUnchanged(paymentOrder.id, totalReserved, {
...
export async function refundPaymentForBooking(params: {
	...
	const paymentOrder = await getLatestPaidPaymentOrderForBooking(
		params.bookingId,
	);
	if (!paymentOrder) {
		throw new ApiException(
...
export function verifyClientPaymentSignature(input: {
	...
	if (!valid) {
		throw new ApiException(
			400,
			"invalid_signature",
			"Client payment signature mismatch.",
		);
	}
